...
`SOURCE_FILES.csv` maps released files to preserved source records, source identifiers, SHA-256 digests and conversions. Rows marked `manuscript-derived-20260911` or `derived-results-YYYYMMDD` are derived results saved for the revised manuscript, rather than a Git commit identifier. Source labels identify recorded inputs; the supplied measurements are self-contained. The label `measurement-record` identifies a recorded experimental input rather than a public Git revision.
...
Version 1.0.3 adds 3 files under `controlled-results/`:

- `iteration_percentile_comparison.csv`: 18 rows covering 3 percentiles for each source/set pair. `signatures` and `nearest_rank` identify the empirical order statistic. `empirical_quantile`, `corrected_reference` and `difference` are iteration counts. `iid_bootstrap_*` describes independent resampling of individual signatures; `cluster_bootstrap_*` describes resampling the 10 shared device keys. The means and standard errors describe the conditional bootstrap distributions. `*_ci_lo` and `*_ci_hi` are pointwise 95% percentile intervals. Host cluster fields are blank because those records do not provide this shared-key design.
- `iteration_cluster_bootstrap_distribution.csv`: the exact device bootstrap probability mass for each set, percentile and possible quantile. `ordered_key_resamples` counts the ordered draws producing that value, out of 10^10 possible draws; `probability` divides by that total. The distribution is computed by enumerating 92,378 distinct key-multiplicity vectors, weighted by their multinomial counts.
- `iteration_percentile_notes.json`: input selection, estimators, bootstrap formulas, weighting and conditioning. A shared key is resampled jointly across every board, message and boot. Bootstrap percentile endpoints use the inverse discrete CDF, without interpolation. No random seed or simulation size is needed because both bootstrap distributions are evaluated directly.

All 9 device percentiles equal their corrected per-set references. Their 99.9th-percentile key-bootstrap intervals are [27, 27], [32, 33] and [23, 25] for sets 44, 65 and 87. A zero-width interval describes concentration in the discrete resampling distribution; it does not imply zero uncertainty about other keys or workloads. The ordinary-bootstrap comparison is a sensitivity analysis and does not replace key clustering for the primary device intervals.
